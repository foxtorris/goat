...
const originAgentRole = "You are a helpful assistant that can use tools to complete tasks."

const originAgentSkillsOverview = `You have access to skills for specialized tasks.
...
const originAgentSkillPriority = `1. User instructions
2. Skill instructions
3. General system/tool rules
4. General assistant behavior`

const originAgentPlanningOverview = `You have access to two planning tools:
...
const originAgentPlanningResponsibility = `You are responsible for constructing the plan yourself before calling 'generate_plan'.
...
const originAgentPlanningGoals = `Planning helps you:
...
const originAgentPlanningRules = `- After selecting the relevant skills, you MUST evaluate whether planning is needed.
...
const originAgentPlanGranularity = `- Default to 3-7 plan steps for ordinary multi-step tasks.
...
const originAgentPlanImmediatelyBoundary = `Create a plan immediately when:
...
const originAgentGatherInformationBoundary = `Gather information before creating a plan when:
...
const originAgentPlanConstructionRules = `When planning is needed:
...
const originAgentRecommendedPlanJSON = `Your structured JSON for 'generate_plan' should usually include, when applicable:
...
const originAgentPlanningWorkflow = `When planning is needed, follow this workflow:
...
const originAgentMandatoryPlanUpdate = `- After completing each item in the plan, calling 'update_plan' is REQUIRED.
...
const originAgentPlanQuality = `A good plan should:
...
const originAgentPlanningAndSkills = `- Skills and planning serve different purposes:
...
const originAgentPlanningAndTools = `- If multiple tools are likely needed, planning is usually appropriate.
...
const originAgentPlanningUserCommunication = `- Do not ask the user for permission to use planning tools unless the user explicitly requests control over that.
...
// OriginAgentSystemPromptTemplate is the non-planning prompt template. Its two
// formatting arguments are the available skills and skill-usage instructions.
var OriginAgentSystemPromptTemplate = buildOriginAgentPrompt(false, "%s", "%s", "")

// OriginAgentWithPlanSystemPromptTemplate is the planning prompt template. Its
// three formatting arguments are the available skills, skill-usage
// instructions, and plan-usage instructions.
var OriginAgentWithPlanSystemPromptTemplate = buildOriginAgentPrompt(true, "%s", "%s", "%s")

func buildOriginAgentPrompt(planMode bool, availableSkills, skillUsageInstruction, planUsageInstruction string) string {
	...
	builder.
		Section("Skills", originAgentSkillsOverview).
		ListSection(
			"How to Load Skills",
		...
		Section("Priority Order", originAgentSkillPriority).
...
func addPlanningPrompt(builder *prompt.Builder, planUsageInstruction string) {
	builder.
		Section("Planning", originAgentPlanningOverview).
		Section(
		...
		Section("Planning Responsibility", originAgentPlanningResponsibility).
		Section("Planning Goals", originAgentPlanningGoals).
		Section("Planning Rules", originAgentPlanningRules).
		Section("Plan Granularity", originAgentPlanGranularity).
		Section("Boundary: When to Plan Immediately", originAgentPlanImmediatelyBoundary).
		Section("Boundary: When to Gather Information First", originAgentGatherInformationBoundary).
		Section("Plan Construction Rules", originAgentPlanConstructionRules).
		Section("Recommended Plan JSON Content", originAgentRecommendedPlanJSON).
		Section("Planning Workflow", originAgentPlanningWorkflow).
		Section("Mandatory Update Rule", originAgentMandatoryPlanUpdate).
		Section("Plan Quality Guidelines", originAgentPlanQuality).
		Section("Relationship Between Planning and Skills", originAgentPlanningAndSkills).
		Section("Relationship Between Planning and Tools", originAgentPlanningAndTools).
		Section("User Communication", originAgentPlanningUserCommunication)
}

func renderOriginAgentSystemPrompt(
	planMode bool,
	skills []string,
